...
public class DefensiveStrategy extends Strategy {
	...
	public void gameTurn() {
		...
		i = 0;
		while ((celldef.getCellType() == Cell.wallCell || celldef.getCellType() == Cell.waterCell
...
//								System.out.println(7);
								...
								if (pathsave1 != null) {
									...
									int min = Math.min(pathsave1.size() - 1, tempPM);
									if (this.attackable(pathsave1.get(pathsave1.size() - 1 - min), cellsave)) {
										this.attack(cellsave);
									}
									tempPM -= pathsave1.size() - 1;
									play = true;
								...
								if (play) {
									...
									play = false;
									while (!play && i < 100) {
										if (this.chara.getPos().distanceFrom(celldef) <= tempPM && celldef.getCellType() != 1
