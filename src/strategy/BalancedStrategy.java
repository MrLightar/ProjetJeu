...
public class BalancedStrategy extends Strategy{
	...
	public void gameTurn() {
		...
		//	si ennemi achevable
		int distance=1000;
		for(Cell enemie:enemiesInRange) {
			if(enemie.getChara().isKillable(Att)) {
				pathsave1=evaluatePathAttack(chara.getPos(), enemie);
				if(pathsave1!=null && pathsave1.size()<distance && attackable(pathsave1.get(0), enemie)) {
					cellsave=enemie;
					pathsave2=pathsave1;
		...
		if(play) {
			...
			applyPath(pathsave2, tempPM);
			System.out.println("on attaque"+cellsave);
			int min=Math.min(pathsave2.size()-1, tempPM);
			if(attackable(pathsave2.get(pathsave2.size()-1-min), cellsave)) {
				attack(cellsave);
			}
			tempPM-=distance-1;
		...
		//	sinon 
			...
			//sinon 
				...
				distance=1000;
				for(Cell bonus:bonusesInRange) {
					pathsave1=evaluatePath(chara.getPos(), bonus);
		...
		if(tempPM>0) {
			...
			if(tempPM>0) {
				...
				analyseMap(tempPM);
				defensSort(rangeOfActionBonuses);
				celldef=rangeOfActionBonuses.get(0);
				i=0;
				while (!play && i < 100) {
					if (current!=null && current.distanceFrom(celldef) <= tempPM && celldef.getCellType() != 1 && celldef.getCellType() != 2
